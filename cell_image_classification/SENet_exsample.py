...
class SEResNeXt(object):
    def __init__(self, size=96, num_classes=4, depth=8, reduction_ratio=4, num_split=2, num_block=3):
        self.depth = depth  # number of channels
        self.ratio = reduction_ratio  # ratio of channel reduction in SE module
        self.num_split = num_split  # number of splitting trees for ResNeXt (so called
        # cardinality)
    ...
    def build_model(self, inputs, num_classes):
        ...
        x = self.residual_layer(x, out_dim=8)
        x = self.residual_layer(x, out_dim=16)
        x = self.residual_layer(x, out_dim=32)
